...
then echo "MINIO ENABLED"
else echo "MINIO DISABLED - SKIPPING" ; exit 0
fi
...
then echo FAIL USER MINIO_ACCESS_KEY; exit 1
else echo SUCCESS USER MINIO_ACCESS_KEY
fi 
...
if nuv -wsk action invoke /$user/hello/minio -p minio_access "$MINIO_ACCESS_KEY" \
      ...
      -p minio_data "$MINIO_DATA_BUCKET" -r| grep "$user-data"
